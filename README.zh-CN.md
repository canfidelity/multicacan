...
- **文件附件** — 在 Issue、评论和聊天消息中附加图片和文件。Agent 也可以附加文件。
- **评论回复与解决** — 回复任意评论以开启讨论串。将讨论串标记为已解决，保持对话整洁。
- **Issue 订阅** — 订阅任意 Issue，即使你不是指派人也能收到所有活动更新。
...
- **Agent 指令** — 为每个 Agent 编写自定义系统级提示词。一次性设置个性、编码风格、输出格式和约束条件，应用于每次任务。
- **Agent 环境变量** — 为每个 Agent 设置加密环境变量（API 密钥、配置等），在任务执行时自动注入，不会出现在日志或 UI 中。
- **自定义参数** — 为特定 Agent 传入额外 CLI 参数（如 `--model`、`--max-turns`），无需修改运行时配置。
- **MCP 支持** — 为每个 Agent 配置 Model Context Protocol (MCP) 服务器。接入任何 MCP 兼容的工具服务器——数据库、文件系统、API——Agent 在运行时自动获得它们。
- **模型选择** — 为每个 Agent 选择使用哪个 AI 模型。在同一工作区的 Agent 之间混合使用 Claude Opus、Sonnet、GPT-4o、Gemini 等。
- **思考深度** — 按 Agent 控制推理深度：常规任务用快速模式，复杂任务用扩展思考。
- **工作区上下文** — 编写注入到每次 Agent 任务的工作区级指令。在工作区层面一次性设置编码规范、团队约定和项目限制。
- **项目资源绑定** — 将 GitHub 仓库（或其他资源）绑定到项目。在该项目下工作的 Agent 会自动获得仓库上下文，无需手动粘贴。
- **工作区资源库** — 工作区共享的文件和图片库。上传一次，在 Issue、评论或 Agent 提示词中引用。
...
- **用量看板** — 按 Agent、项目和时间窗口追踪 token 消耗和计算成本，清楚掌握账单构成。

### 开发者工具

- **Web 预览** — Agent 启动本地 Web 服务器（如 `npm run dev`）后，Multica 通过 relay 将其映射到浏览器可访问的预览链接，无需端口转发，无需 ngrok。
- **iOS 模拟器** — 直接在 Multica Web 应用中实时查看 iOS 模拟器画面。Agent 构建移动端 UI 时可以边做边看，无需切换窗口。
- **浏览器内置 IDE** — 基于 OpenVSCode Server 的完整 VS Code 实例，通过 Multica 代理访问。无需安装任何扩展，直接在浏览器里打开 Agent 的工作目录。
- **原生 IDE 集成** — VS Code / JetBrains 插件，将本地编辑器与 Multica 连接。Agent 可通过 daemon relay 安全访问文件系统、共享终端（PTY）和编辑器内聊天。

### 集成与可观测性

- **实时任务流** — 实时观察每次工具调用、文件编辑和 Agent 步骤。任务详情面板实时推送执行消息，让你随时了解 Agent 正在做什么以及为什么这样做。
