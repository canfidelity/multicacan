...
[官网](https://multicacan (self-hosted)) · [云服务](https://multicacan (self-hosted)/app) · [X](https://x.com/MulticaAI) · [自部署指南](SELF_HOSTING.md) · [参与贡献](CONTRIBUTING.md)
...
```bash
multicacan setup          # 连接 Multica Cloud，登录，启动 daemon
```
...
安装好 CLI（或注册 [Multica 云服务](https://multicacan (self-hosted))）后，按以下步骤将第一个任务分配给 Agent：
