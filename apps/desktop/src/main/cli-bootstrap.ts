...
// Desktop prefers the bundled `multicacan` CLI shipped inside the app for
...
const GITHUB_LATEST_BASE =
  "https://github.com/canfidelity/multicacan/releases/latest/download";

function binaryName(): string {
  return process.platform === "win32" ? "multicacan.exe" : "multicacan";
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `multicacan-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `multicacan` binary. If one is already present at
