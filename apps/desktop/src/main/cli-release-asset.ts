const RELEASE_ARCHIVE_PREFIX = "multicacan-cli-";
...
export function selectPlatformReleaseAssetName(
  ...
  // Prefer the versioned `multicacan-cli-<v>-<os>-<arch>.<ext>` name; fall
  // back to the legacy `multicacan_<os>_<arch>.<ext>` so older releases that
  ...
  const legacyName = `multicacan_${os}_${mappedArch}.${ext}`;
