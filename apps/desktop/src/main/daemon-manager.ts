...
const PREFS_PATH = join(homedir(), ".multicacan", "desktop_prefs.json");
...
// Keep the Go impl in sync: server/cmd/multicacan/cmd_daemon.go healthPortForProfile.
...
function profileDir(profile: string): string {
  return profile
    ? join(homedir(), ".multicacan", "profiles", profile)
    : join(homedir(), ".multicacan");
}
...
// Sidecar file that records which Multicacan user the cached PAT in config.json
...
//   ~/.multicacan/profiles/desktop-<host>/
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["multicacan.exe"] : ["multicacan"];
 ...
 *   to `apps/desktop/resources/bin/multicacan`. `bundle-cli.mjs` populates this
 *   before dev starts, so iterating on Go changes is "make build → restart".
 * - Packaged: `app.getAppPath()` → `<Multicacan.app>/Contents/Resources/app.asar`.
...
function bundledCliPath(): string {
  const binName = process.platform === "win32" ? "multicacan.exe" : "multicacan";
...
/**
 * Returns a usable `multicacan` binary path. Priority:
 ...
 *   5. `multicacan` on PATH (dev convenience / user-installed via brew).
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "Multicacan Desktop" }),
  });
  if (!res.ok) {
...
async function savePrefs(prefs: DaemonPrefs): Promise<void> {
  const dir = join(homedir(), ".multicacan");
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  return { ...process.env, MULTICACAN_LAUNCHED_BY: "desktop" };
}

async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "multicacan CLI is not installed" };
