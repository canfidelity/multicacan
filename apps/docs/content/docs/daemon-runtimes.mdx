...
description: Agents don't run on Multicacan's servers — they run on your own machines.
---

import { Callout } from "fumadocs-ui/components/callout";
import { Mermaid } from "@/components/mermaid";

In Multicacan, [agents](/agents) do **not** run on our servers — they run on your own machines, driven by a small program called the **daemon** that invokes the [AI coding tools](/providers) installed locally. The Multicacan server only coordinates: it stores [issues](/issues), queues [tasks](/tasks), and dispatches them to the right **runtime** (runtime = daemon × one AI coding tool).

This structure is the biggest difference between Multicacan and Linear / Jira: **your API keys, toolchain, and code directories stay on your machine** — the Multicacan server never sees any of them. That means "my agent isn't working" is almost always a local problem — the daemon isn't running, an AI tool isn't installed, a key has expired. Check locally first; see [Troubleshooting](/troubleshooting) for a guide.

## Starting the daemon

The daemon is part of the Multicacan CLI. Once you've installed the [Multicacan CLI](/cli), run on your own machine:
...
| Command | Purpose |
|---|---|
| `multicacan daemon start` | Start (background by default; add `--foreground` to run in the foreground) |
| `multicacan daemon stop` | Stop |
| `multicacan daemon restart` | Restart |
| `multicacan daemon status` | Show status |
| `multicacan daemon logs` | Show logs (add `-f` to follow) |

Full CLI reference in [CLI commands](/cli).

**The desktop app ships with a daemon.** If you use the [desktop app](/desktop-app), you don't need to run `multicacan daemon start` manually — it launches the daemon automatically on startup.

## Why one machine has multiple runtimes

A runtime is not a server and not a container — it's the combination of "**daemon × one AI coding tool**". For example: you start the daemon on a MacBook with both Claude Code and Codex installed, and you're a member of two workspaces. Multicacan then registers 4 runtimes:
...
- The **Runtimes** page in the Multicacan UI lists these rows

<Callout type="info">
**Cloud runtimes are coming**, currently in a waitlist phase. Once available, you'll be able to execute agent tasks directly on Multicacan Cloud without running a local daemon. Sign up with your email on the [download page](https://multicacan.ai/download) to get notified.
</Callout>

## When a runtime is marked offline

Multicacan uses heartbeats to decide whether a runtime is online. Three key numbers:
...
**Tasks running on a missing runtime are marked as failed** (failure reason `runtime_offline`). For retryable sources (issues, chat), Multicacan automatically requeues them; Autopilot-triggered tasks are not retried automatically. See [Tasks → Which failures retry automatically](/tasks#which-failures-retry-automatically-which-dont).
</Callout>

## How many tasks can run in parallel

Multicacan enforces concurrency limits at two layers:

- **Daemon layer**: **20 concurrent tasks** by default (tunable via env var `MULTICACAN_DAEMON_MAX_CONCURRENT_TASKS`)
...
1. Run `multicacan daemon status` — confirm the daemon is running and online
2. Run `multicacan daemon logs -f` — check for errors
3. Open the **Runtimes** page in the Multicacan UI — confirm your runtime shows "online"
