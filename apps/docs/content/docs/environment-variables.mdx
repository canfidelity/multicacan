...
|---|---|---|
| `DATABASE_URL` | `postgres://multicacan:multicacan@localhost:5432/multicacan?sslmode=disable` | **Yes** |
| `PORT` | `8080` | No (unless you change the port) |
| `JWT_SECRET` | `multicacan-dev-secret-change-in-production` | **Yes** (the default is unsafe) |
| `APP_ENV` | empty | **Yes** (must be `production`) |
| `FRONTEND_ORIGIN` | empty | **Yes** (self-host must set its own domain) |
| `MULTICACAN_DEV_VERIFICATION_CODE` | empty | No (must stay empty in production) |

<Callout type="warning">
**Keep `MULTICACAN_DEV_VERIFICATION_CODE` empty in production.** A fixed local test code is disabled by default, but if you opt in with `MULTICACAN_DEV_VERIFICATION_CODE=888888`, anyone who can request a code can sign in with that fixed value while `APP_ENV` is non-production. The shortcut is ignored when `APP_ENV=production`.
</Callout>
...
Multicacan uses [Resend](https://resend.com/) to send verification codes and invite emails.

| Variable | Default | Description |
|---|---|---|
| `RESEND_API_KEY` | empty | Resend API key |
| `RESEND_FROM_EMAIL` | `noreply@multicacan.ai` | Sender address (must be a domain verified in your Resend account) |
...
Multicacan stores user-uploaded attachments (images and files in comments). **S3 is preferred**; if S3 is not configured, it falls back to local disk.
...
**Invite flows themselves do not check the signup allowlist** — but the invitee must still be able to **sign in** before accepting the invite. If they already have a Multicacan account (for example from another workspace), they can accept directly, unaffected by the allowlist; **if they have never signed up**, the first step of sign-in (requesting a verification code) still passes through the allowlist check, and an email rejected by `ALLOW_SIGNUP=false` or by `ALLOWED_EMAILS` / `ALLOWED_EMAIL_DOMAINS` **cannot finish signup, and therefore cannot accept the invite**.
...
| Variable | Default | Description |
|---|---|---|
| `MULTICACAN_SERVER_URL` | `ws://localhost:8080/ws` | Server address (self-host: replace with your domain) |
| `MULTICACAN_DAEMON_HEARTBEAT_INTERVAL` | `15s` | Heartbeat interval |
| `MULTICACAN_DAEMON_POLL_INTERVAL` | `3s` | Task polling interval |
| `MULTICACAN_DAEMON_MAX_CONCURRENT_TASKS` | `20` | Max concurrent tasks |
| `MULTICACAN_<PROVIDER>_PATH` | matches the CLI name | Path to each AI coding tool's executable (for example `MULTICACAN_CLAUDE_PATH`) |
| `MULTICACAN_<PROVIDER>_MODEL` | empty | Default model for each AI coding tool |
...
| Variable | Default | Description |
|---|---|---|
| `FRONTEND_ORIGIN` | empty | Frontend address. Invite email links, the CORS allowlist, and the cookie domain are all derived from this. When unset, invite email links fall back to the hosted domain `https://app.multicacan.ai` — self-host must set this explicitly |
...
<Callout type="warning">
**Leaving `FRONTEND_ORIGIN` unset creates two silent failures**: (1) invite email links point at `https://app.multicacan.ai` (the hosted domain), and clicking them doesn't bring users back to your self-hosted instance; (2) WebSocket Origin checks fall back to `localhost:3000 / 5173 / 5174`, so every WebSocket connection in a production deployment is rejected and the frontend appears to "lose real-time updates."
</Callout>

## Usage analytics

By default, the server reports to Multicacan's official PostHog instance. To opt out, set `ANALYTICS_DISABLED=true`.
...
- [Daemon and runtimes](/daemon-runtimes) — what the `MULTICACAN_DAEMON_*` parameters actually do
