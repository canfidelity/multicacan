...
description: Deploy Multicacan on your own infrastructure.
---

## Architecture Overview

Multicacan has three components:
...
Each user who wants to run AI agents locally also installs the **`multicacan` CLI** and runs the **agent daemon** on their own machine.
...
curl -fsSL https://raw.githubusercontent.com/multicacan-ai/multicacan/main/scripts/install.sh | bash -s -- --with-server

# Configure CLI, authenticate, and start the daemon
multicacan setup self-host
```

This installs the CLI, checks out the latest self-host assets, pulls the official Multicacan images from GHCR, and configures everything for localhost. Then open http://localhost:3000 and pick a login method: configure `RESEND_API_KEY` in `.env` for email-based codes (recommended), or leave Resend unset and copy the generated code from backend logs. See [Step 2 — Log In](#step-2--log-in) for details.

<Callout>
If the self-host server is already running and you only need the CLI on a macOS/Linux machine, install it with Homebrew: `brew install multicacan-ai/tap/multicacan`.
</Callout>
...
<Callout>
Alternatively, configure step by step: `multicacan config set server_url http://localhost:8080 && multicacan config set app_url http://localhost:3000 && multicacan login && multicacan daemon start`
</Callout>
...
If you've been self-hosting and want to switch your CLI to [Multicacan Cloud](https://multicacan.ai):

```bash
multicacan setup
```

This reconfigures the CLI for multicacan.ai, re-authenticates, and restarts the daemon. You will be prompted before overwriting the existing configuration.
...
| `RESEND_FROM_EMAIL` | Sender email address (default: `noreply@multicacan.ai`) |
...
This starts a `pgvector/pgvector:pg17` container on port 5432 with default credentials (`multicacan`/`multicacan`).
