...
describe("LoginPage", () => {
  ...
  it("renders email form with 'Sign in to Multicacan' title", () => {
    render(<LoginPage onSuccess={onSuccess} />);
    expect(
      screen.getByText(/sign in to multicacan/i),
    ).toBeInTheDocument();
    expect(
  ...
  it("shows cli_confirm step when existing session + cliCallback", async () => {
    localStorage.setItem("multicacan_token", "existing-jwt");
