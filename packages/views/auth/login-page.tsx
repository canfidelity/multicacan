...
import {
  ...
  InputOTPGroup,
  InputOTPSlot,
} from "@multicacan/ui/components/ui/input-otp";
...
import type { User } from "@multicacan/core/types";
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch(() => {
        ...
        const token = localStorage.getItem("multicacan_token");
        if (!token) return;
        ...
        api
          ...
          .catch(() => {
            api.setToken(null);
            localStorage.removeItem("multicacan_token");
          });
      });
  }, [cliCallback]);
    ...
    async (value: string) => {
      ...
      try {
        if (cliCallback) {
          ...
          localStorage.setItem("multicacan_token", token);
          api.setToken(token);
          onTokenObtained?.();
  ...
  const handleCliAuthorize = async () => {
    ...
    try {
      ...
      if (authSourceRef.current === "localStorage") {
        ...
        const stored = localStorage.getItem("multicacan_token");
      ...
      <div className="flex min-h-svh items-center justify-center">
        <Card className="w-full max-w-sm">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Allow the CLI to access Multicacan as{" "}
  ...
  return (
    <div className="flex min-h-svh items-center justify-center">
      <Card className="w-full max-w-sm">
        <CardHeader className="text-center">
          {logo && <div className="mx-auto mb-4">{logo}</div>}
          <CardTitle className="text-2xl">Sign in to Multicacan</CardTitle>
          <CardDescription>
