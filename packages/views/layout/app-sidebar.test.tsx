...
import { ApiError } from "@multicacan/core/api";
...
vi.mock("@multicacan/ui/components/ui/sidebar", () => ({
...
vi.mock("@multicacan/ui/components/ui/dropdown-menu", () => ({
...
vi.mock("@multicacan/ui/components/common/actor-avatar", () => ({ ActorAvatar: () => <span /> }));

vi.mock("@multicacan/core/auth", () => ({
  useAuthStore: (selector: (state: { user: { id: string } }) => unknown) => selector({ user: { id: "user-1" } }),
}));
vi.mock("@multicacan/core/paths", () => ({
...
vi.mock("@multicacan/core/api", async (importOriginal) => ({ ...(await importOriginal<typeof import("@multicacan/core/api")>()), api: {} }));
vi.mock("@multicacan/core/inbox/queries", () => ({ deduplicateInboxItems: (items: unknown[]) => items, inboxKeys: { list: () => ["inbox"] } }));
vi.mock("@multicacan/core/issues/queries", () => ({ issueDetailOptions: () => ({ queryKey: ["issue"] }) }));
vi.mock("@multicacan/core/issues/stores/create-mode-store", () => ({
  useCreateModeStore: { getState: () => ({ lastMode: "agent" }) },
  openCreateIssueWithPreference: vi.fn(),
}));
vi.mock("@multicacan/core/issues/stores/draft-store", () => ({ useIssueDraftStore: () => false }));
