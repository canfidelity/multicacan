...
const {
  ...
  mockGetShareableUrl: vi.fn((p: string) => `https://app.multicacan/${p}`),
  mockWorkspaces: {
...
vi.mock("@multicacan/ui/components/common/theme-provider", () => ({
...
describe("SearchCommand", () => {
  beforeEach(() => {
    ...
    mockGetShareableUrl.mockReset().mockImplementation((p: string) => `https://app.multicacan/${p}`);
  ...
  it("copies issue link and identifier when on an issue detail route", async () => {
    ...
    expect(mockClipboardWrite).toHaveBeenCalledWith("https://app.multicacan//ws-test/issues/issue-1");
