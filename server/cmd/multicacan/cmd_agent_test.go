...
// agent task (MULTICACAN_AGENT_ID / MULTICACAN_TASK_ID set), the CLI must NOT
// silently read the user-global ~/.multicacan/config.json to recover a missing
...
func TestResolveWorkspaceID_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("outside agent context falls back to config", func(t *testing.T) {
		t.Setenv("MULTICACAN_AGENT_ID", "")
		t.Setenv("MULTICACAN_TASK_ID", "")
	...
	t.Run("agent context with explicit env uses env", func(t *testing.T) {
		t.Setenv("MULTICACAN_AGENT_ID", "agent-123")
		t.Setenv("MULTICACAN_TASK_ID", "task-456")
		t.Setenv("MULTICACAN_WORKSPACE_ID", "env-ws")
...
func TestAgentUpdateNoFieldsErrorPointsAtEnvCommand(t *testing.T) {
	...
	t.Setenv("MULTICACAN_WORKSPACE_ID", "test-ws")
