...
func init() {
	...
	f.String("daemon-id", "", "Unique daemon identifier (env: MULTICACAN_DAEMON_ID)")
	f.String("device-name", "", "Human-readable device name (env: MULTICACAN_DAEMON_DEVICE_NAME)")
	f.String("runtime-name", "", "Runtime display name (env: MULTICACAN_AGENT_RUNTIME_NAME)")
	f.Duration("poll-interval", 0, "Task poll interval (env: MULTICACAN_DAEMON_POLL_INTERVAL)")
	f.Duration("heartbeat-interval", 0, "Heartbeat interval (env: MULTICACAN_DAEMON_HEARTBEAT_INTERVAL)")
	f.Duration("agent-timeout", 0, "Per-task timeout (env: MULTICACAN_AGENT_TIMEOUT)")
	f.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: MULTICACAN_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	f.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: MULTICACAN_DAEMON_MAX_CONCURRENT_TASKS)")
	f.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: MULTICACAN_DAEMON_AUTO_UPDATE=false)")
	f.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: MULTICACAN_DAEMON_AUTO_UPDATE_INTERVAL)")
	...
	rf.String("daemon-id", "", "Unique daemon identifier (env: MULTICACAN_DAEMON_ID)")
	rf.String("device-name", "", "Human-readable device name (env: MULTICACAN_DAEMON_DEVICE_NAME)")
	rf.String("runtime-name", "", "Runtime display name (env: MULTICACAN_AGENT_RUNTIME_NAME)")
	rf.Duration("poll-interval", 0, "Task poll interval (env: MULTICACAN_DAEMON_POLL_INTERVAL)")
	rf.Duration("heartbeat-interval", 0, "Heartbeat interval (env: MULTICACAN_DAEMON_HEARTBEAT_INTERVAL)")
	rf.Duration("agent-timeout", 0, "Per-task timeout (env: MULTICACAN_AGENT_TIMEOUT)")
	rf.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: MULTICACAN_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	rf.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: MULTICACAN_DAEMON_MAX_CONCURRENT_TASKS)")
	rf.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: MULTICACAN_DAEMON_AUTO_UPDATE=false)")
	rf.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: MULTICACAN_DAEMON_AUTO_UPDATE_INTERVAL)")
...
// Empty profile → ~/.multicacan/, named profile → ~/.multicacan/profiles/<name>/.
...
func runDaemonForeground(cmd *cobra.Command) error {
	...
	serverURL := cli.FlagOrEnv(cmd, "server-url", "MULTICACAN_SERVER_URL", "")
	if serverURL == "" {
	...
	cfg.LaunchedBy = os.Getenv("MULTICACAN_LAUNCHED_BY")
