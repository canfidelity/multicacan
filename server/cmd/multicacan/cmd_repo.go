...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("MULTICACAN_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("MULTICACAN_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("MULTICACAN_WORKSPACE_ID")
	agentName := os.Getenv("MULTICACAN_AGENT_NAME")
