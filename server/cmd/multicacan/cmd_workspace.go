...
func fetchWorkspaces(ctx context.Context, cmd *cobra.Command) ([]workspaceSummary, error) {
	...
	if token == "" {
		return nil, fmt.Errorf("not authenticated: run 'multicacan login' first")
	}
...
func runWorkspaceList(cmd *cobra.Command, _ []string) error {
	...
	if currentID != "" {
		fmt.Fprintln(os.Stderr, "\n* = current default workspace (use 'multicacan workspace switch <id|slug|prefix>' to change)")
	} else {
		fmt.Fprintln(os.Stderr, "\nNo default workspace set. Use 'multicacan workspace switch <id|slug|prefix>' to pick one.")
	}
...
func resolveWorkspaceByIDOrSlug(workspaces []workspaceSummary, target string) (workspaceSummary, error) {
	...
	return workspaceSummary{}, fmt.Errorf("workspace %q not found or you do not have access; run 'multicacan workspace list' to see options", target)
}
...
func runWorkspaceGet(cmd *cobra.Command, args []string) error {
	...
	if wsID == "" {
		return fmt.Errorf("workspace ID is required: pass an id/slug/prefix as argument or set MULTICACAN_WORKSPACE_ID")
	}
