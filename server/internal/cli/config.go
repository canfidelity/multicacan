...
const defaultCLIConfigPath = ".multicacan/config.json"
...
// An empty profile returns the default path (~/.multicacan/config.json).
// A named profile returns ~/.multicacan/profiles/<name>/config.json.
func CLIConfigPathForProfile(profile string) (string, error) {
	...
	return filepath.Join(home, ".multicacan", "profiles", profile, "config.json"), nil
}

// ProfileDir returns the base directory for a profile's state files (pid, log).
// An empty profile returns ~/.multicacan/. A named profile returns ~/.multicacan/profiles/<name>/.
func ProfileDir(profile string) (string, error) {
	...
	if profile == "" {
		return filepath.Join(home, ".multicacan"), nil
	}
	return filepath.Join(home, ".multicacan", "profiles", profile), nil
}
