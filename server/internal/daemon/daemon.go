...
func (d *Daemon) triggerRestart() {
	...
	// must use the stable <brew-prefix>/bin/multicacan symlink instead.
	if isBrewInstall() {
		if brewPrefix := getBrewPrefix(); brewPrefix != "" {
			newBin = filepath.Join(brewPrefix, "bin", "multicacan")
		} else if prefix := matchKnownBrewPrefix(newBin); prefix != "" {
			newBin = filepath.Join(prefix, "bin", "multicacan")
		} else {
