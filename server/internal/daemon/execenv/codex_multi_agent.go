...
// subagents. The Multicacan daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Multicacan runtime
...
//
// Users who explicitly want Codex native subagents inside a Multicacan task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `MULTICACAN_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// MulticacanCodexMultiAgentEnv is the env var users can set to keep Codex
...
const MulticacanCodexMultiAgentEnv = "MULTICACAN_CODEX_MULTI_AGENT"

// multicacanMultiAgentBeginMarker / multicacanMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	multicacanMultiAgentBeginMarker = "# BEGIN multicacan-managed multi-agent (do not edit; regenerated by daemon)"
	multicacanMultiAgentEndMarker   = "# END multicacan-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(multicacanMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(multicacanMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(MulticacanCodexMultiAgentEnv))
...
// renderMulticacanMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderMulticacanMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(multicacanMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(multicacanMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderMulticacanMultiAgentBlock(true)
...
//
// When MULTICACAN_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per MULTICACAN_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderMulticacanMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
