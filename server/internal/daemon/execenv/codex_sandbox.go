...
// `no such host` errors when calling out (for example, `multicacan issue get`
// hitting the Multicacan API). See upstream issue openai/codex#10390.
//
...
// actually reach the Multicacan API. On Linux (and on macOS once the upstream
...
//   danger-full-access so the Multicacan CLI can reach the API.
...
// multicacanManagedBeginMarker / multicacanManagedEndMarker delimit the block the
...
const (
	multicacanManagedBeginMarker = "# BEGIN multicacan-managed (do not edit; regenerated by daemon)"
	multicacanManagedEndMarker   = "# END multicacan-managed"
)

// renderMulticacanManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.multicacan]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertMulticacanManagedBlock), avoids both traps.
func renderMulticacanManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(multicacanManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(multicacanManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(multicacanManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(multicacanManagedEndMarker) + `\n*`)

// upsertMulticacanManagedBlock returns the config content with the multicacan-managed
...
// `[permissions.multicacan]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderMulticacanManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertMulticacanManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderMulticacanManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the multicacan-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertMulticacanManagedBlock(existing, policy)
