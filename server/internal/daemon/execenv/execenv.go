...
// checked out on demand by the agent via `multicacan repo checkout`.
package execenv
...
type PrepareParams struct {
	WorkspacesRoot string            // base path for all envs (e.g., ~/multicacan_workspaces)
...
type TaskContextForEnv struct {
	...
	ProjectModelPool        string // JSON array of available models for the project; injected into leader brief
...
// on demand via `multicacan repo checkout <url>`.
