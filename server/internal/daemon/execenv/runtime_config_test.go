	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, want := range []string{
				...
				"`--status backlog` = **wait**",
				"`multicacan issue status <child-id> todo`",
				"all `--status todo`",
...
func TestBriefHasNoParentNotificationGuidance(t *testing.T) {
	...
	for _, ctx := range cases {
		...
		for _, banned := range []string{
			...
			"multicacan issue comment add <parent-id>",
			"with NO `--parent`",
			...
			"`multicacan issue status <this-issue-id> in_review`",
...
// dedicated protocol step that wrote `multicacan issue status <this-issue-id> in_review`;
...
func TestCommentTriggeredProtocolDoesNotForceInReview(t *testing.T) {
	...
	if strings.Contains(out, "`multicacan issue status <this-issue-id> in_review`") {
...
func TestAssignmentTriggeredProtocolStillFlipsInReview(t *testing.T) {
	...
	want := "`multicacan issue status " + issueID + " in_review`"
	if !strings.Contains(out, want) {
