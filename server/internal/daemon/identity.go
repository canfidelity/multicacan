...
// same machine shares one UUID stored at `~/.multicacan/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.multicacan/profiles/*/daemon.id` and returns every
...
// explicitly pins MULTICACAN_DAEMON_ID to the hostname itself, there's nothing
