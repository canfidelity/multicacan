...
func TestEnsureDaemonID_Persists(t *testing.T) {
	...
	path := filepath.Join(home, ".multicacan", "daemon.id")
...
func TestEnsureDaemonID_SharedAcrossProfiles(t *testing.T) {
	...
	// only source of truth is ~/.multicacan/daemon.id.
	profileFile := filepath.Join(home, ".multicacan", "profiles", "staging", "daemon.id")
...
func TestEnsureDaemonID_PromotesPreChangeProfileFile(t *testing.T) {
	...
	profileDir := filepath.Join(home, ".multicacan", "profiles", "staging")
	...
	data, err := os.ReadFile(filepath.Join(home, ".multicacan", "daemon.id"))
	if err != nil {
...
func TestEnsureDaemonID_RegeneratesCorruptFile(t *testing.T) {
	...
	dir := filepath.Join(home, ".multicacan")
...
func TestLegacyDaemonUUIDs_ScansProfileDirs(t *testing.T) {
	...
	for name, id := range map[string]string{"prod": uuidA, "desktop-multicacan": uuidB} {
		dir := filepath.Join(home, ".multicacan", "profiles", name)
		if err := os.MkdirAll(dir, 0o755); err != nil {
	...
	corruptDir := filepath.Join(home, ".multicacan", "profiles", "corrupt")
