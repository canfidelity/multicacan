...
// the multicacan CLI version reported by the daemon at registration time. The
...
// `multicacan issue create` failure — see PR #1851); older daemons would either
...
var (
	ErrCLIVersionMissing = errors.New("multicacan CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("multicacan CLI version is below required minimum")
)
